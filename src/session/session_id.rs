...
impl SessionId {
  ...
  pub fn from_map(prop_map: &HashMap<String, String>, defaults: &HashMap<String, String>) -> Self {
    let mut builder = SessionIdBuilder::default();
    builder
      .begin_string(
        prop_map.get(BEGIN_STRING_SETTING).or_else(|| defaults.get(BEGIN_STRING_SETTING)).unwrap(),
      )
      .sender_compid(
        prop_map
    ...
    if let Some(sender_loc) =
      prop_map.get(SENDER_LOCATIONID_SETTING).or_else(|| defaults.get(SENDER_LOCATIONID_SETTING))
    {
    ...
    if let Some(target_loc) =
      prop_map.get(TARGET_LOCATIONID_SETTING).or_else(|| defaults.get(TARGET_LOCATIONID_SETTING))
    {
