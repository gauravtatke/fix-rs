...
impl Properties {
  ...
  pub fn get_config<'a, F>(&self, session_id: &SessionId, name: &'a str) -> Result<F, ConfigErr<'a>>
  where
    F: FromStr,
    <F as FromStr>::Err: Debug,
  {
  ...
  where
    ...
    <F as FromStr>::Err: Debug,
  {
    let set_value = self.get_property(session_id, name).or_else(|| self.get_default_property(name));
    let parsed_val = match set_value {
      Some(str_val) => {
        str_val.parse::<F>().map_err(|er| ConfigErr::ParseError(format!("{:?}", er)))
      }
  ...
  fn check(&self) {
    ...
    for session_id in self.session_ids() {
      ...
      if connection_type == ACCEPTOR_CONN_TYPE {
        ...
        if self.get_optional_config::<String>(session_id, SOCKET_CONNECT_HOST_SETTING).is_none()
          || self.get_optional_config::<u16>(session_id, SOCKET_CONNECT_PORT_SETTING).is_none()
        {
