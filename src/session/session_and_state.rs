...
impl Session {
  ...
  pub fn with_settings(session_id: &SessionId, session_setting: &Properties) -> Self {
    ...
    let heartbeat_interval: u32 =
      session_setting.get_optional_config(session_id, HEARTBEAT_INTERVAL_SETTING).unwrap_or(30);
    ...
    let reset_on_logout: bool =
      session_setting.get_optional_config(session_id, RESET_ON_LOGOUT_SETTING).unwrap_or(true);
    let reset_on_disconnect: bool =
      session_setting.get_optional_config(session_id, RESET_ON_DISCONNECT_SETTING).unwrap_or(true);
