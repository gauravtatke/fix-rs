...
impl DataDictionary {
  ...
  fn set_field_for(&mut self, msg_type: &str, fnum: u32, required: bool) -> DResult<()> {
    ...
    if msg_fields.contains(&fnum) {
      return Err(XmlError::DuplicateField(format!("field {} in message {}", fnum, msg_type)));
    }
    msg_fields.insert(fnum);
    if required {
      self.msg_required_fields.entry(msg_type.to_owned()).or_insert_with(HashSet::new).insert(fnum);
    }
    Ok(())
  }
    ...
    for grp_child in group_node.children().filter(|&n| n.is_element()) {
      ...
      let first_field: u32 = match child_name {
        ...
        "component" => {
          ...
          group_dd.add_xml_component(msg_type, comp_node, comp_required, components, doc)?
        }
        "group" => {
          ...
          let field = group_dd.add_fields_to(msg_type, sub_group_name, is_grp_req, doc)?;
...
fn get_attribute<'a>(attr: &str, node: &Node<'a, '_>) -> DResult<&'a str> {
  let requested = match node.attribute(attr) {
    ...
    None => {
      return Err(XmlError::AttributeNotFound(format!("{} in {}", attr, node.tag_name().name())));
    }
  };
  Ok(requested)
}
...
mod dictionary_tests {
  ...
  lazy_static! {
    ...
    static ref DOC: Document<'static> = Document::parse(&XML).expect("test document parse error");
  ...
  fn get_all_field_values() -> HashMap<u32, HashSet<String>> {
    ...
    let fields = lookup_node("fields", &DOC).unwrap();
    for fnode in fields.children().filter(|node| node.is_element() && node.has_tag_name("field")) {
  ...
  fn get_only_fields_for_msg_type(msg_type: &str, doc: &Document) -> HashMap<String, bool> {
    ...
    let msg_fields = msg_node
      .children()
      .filter(|node| node.is_element() && node.has_tag_name("field"))
      .map(|node| (get_name_attr(&node).unwrap().to_string(), get_required_attr(&node).unwrap()))
      .collect();
    msg_fields
  }
